...
public class UserProcess {
	...
	// for now, just assume that virtual addresses=physical addresses
	if (vaddr < 0 || vaddr >= memory.length)
	    return 0;
	...
	for (int s=0; s<coff.getNumSections(); s++) {
		...
		// for now, just assume virtual addresses equal physical addresses
    ...
    private int handleRead(int fileDescriptor, int bufferVAddr, int count) {
        fdLock.lock();
        try {
            if (fileDescriptor < 0 || fileDescriptor >= MAX_OPEN_FILES || !fileDescriptors.containsKey(fileDescriptor)) {
                return -1;
            }
    ...
    private int handleWrite(int fileDescriptor, int bufferVAddr, int count) {
        ...
        try {
            ...
            OpenFile file = fileDescriptors.get(fileDescriptor);
            byte[] buffer = new byte[count];
            int bytesRead = readVirtualMemory(bufferVAddr, buffer);
    ...
    private int handleClose(int fileDescriptor) {
        ...
        try {
            ...
            OpenFile file = fileDescriptors.remove(fileDescriptor);
            file.close();
            return 0;
        } finally {
            fdLock.unlock();
     ...
     */
    private int handleCreat(int nameVAddr) {
        fdLock.lock();
        try {
            String filename = readVirtualMemoryString(nameVAddr, 256);
            if (filename == null) {
            ...
            int fileDescriptor = getNewFileDescriptor();
            if (fileDescriptor == -1) {
                file.close();
                return -1;
            }
    ...
    private int getNewFileDescriptor() {
        for (int i = 2; i < MAX_OPEN_FILES; i++) {
            if (!fileDescriptors.containsKey(i)) {
                return i;
            }
        }
        return -1;
    ...
    /**
     * Terminate this process. This method is called by the last thread of this
     * process, after all other threads have exited.
     */
    ...
    private Map<Integer, OpenFile> fileDescriptors;
    private int nextFileDescriptor;
    private Lock fdLock;

    public UserProcess() {
        fileDescriptors = new HashMap<>();
        nextFileDescriptor = 2; // 0 and 1 are reserved for stdin and stdout
        fdLock = new ReentrantLock();
